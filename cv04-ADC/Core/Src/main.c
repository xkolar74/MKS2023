...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	while (1)
	{
		// Voltage and temperature calculation from ADC values
		...
		// Go to default state of showing pot. after 1000ms
		if(HAL_GetTick() > lastButtonPress+1000) {
			state = SHOW_POT;
		}

		// Show values based on current state
		switch (state) {
			case SHOW_POT:
